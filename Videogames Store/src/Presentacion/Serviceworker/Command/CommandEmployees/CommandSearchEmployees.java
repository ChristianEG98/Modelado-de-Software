...
public class CommandSearchEmployees implements Command{
...
TEmployees tEmployees= FactorySA.getInstance().createSAEmployees().searchEmployees((int) details);
		...
		if(tEmployees != null) return new Context(Events.SEARCH_EMPLOYEES_OK, tEmployees);
		else return new Context(Events.SEARCH_EMPLOYEES_KO, tEmployees);
	}
